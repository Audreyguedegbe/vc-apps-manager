jQuery(document).ready(function($) {
    function openMediaUploader(inputId, previewId) {
        ...
        customUploader.on('select', function() {
            ...
            $('#' + previewId).html('<img src="' + attachment.url + '" style="max-height: 100px;">');
        });
    ...
    $(document).on('click', '.vc-upload-feature-image', function (e) {
        ...
        customUploader.on('select', function () {
            ...
            $button.siblings('.vc-feature-image-preview').html('<img src="' + attachment.url + '" style="max-height: 100px;">');
        });
    ...
    $('#add-feature-button').on('click', function (e) {
        e.preventDefault();
        const template = $('#vc-feature-template').html().replace(/__index__/g, featureIndex);
    ...
    $('#add-faq-button').on('click', function (e) {
        e.preventDefault();
        const template = $('#vc-faq-template').html().replace(/__index__/g, faqIndex);
        $('#vc_faqs_container').append(template);
        faqIndex++;
    });

    // Supprimer un block feature ou FAQ
    $(document).on('click', '.remove-feature', function (e) {
        e.preventDefault();
        $(this).closest('.vc-feature-block').remove();
    });
    ...
    $.post(ajaxurl, {
        ...
        if (response.success) {
            const newCat = $('<option>').val(response.data.term_id).text(response.data.name);
            $('#vc_app_category').append(newCat);
            $('#vc_app_category').val(response.data.term_id);
            $('#new_category_name').val('');
            $('#cat-msg').text('Catégorie ajoutée !').css('color', 'green').fadeIn().delay(2000).fadeOut();
        } else {
            $('#cat-msg').text('Erreur : ' + response.data.message).css('color', 'red').fadeIn().delay(3000).fadeOut();
        }
    });
});
    ...
    function switchContainer(targetId) {
        ...
        $('.container-menus a[href="' + targetId + '"]').closest('.menu-info').addClass('active');
    }
    ...
    function updatePlatformUrlVisibility() {
            if (multiPlatformSelect && multiPlatformUrlField) {
                multiPlatformUrlField.style.display = multiPlatformSelect.value === 'yes' ? 'block' : 'none';
            }
        }
        updatePlatformUrlVisibility();
if (multiPlatformSelect) multiPlatformSelect.addEventListener('change', updatePlatformUrlVisibility);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let monthlyIndex = document.querySelectorAll('#monthly-plans .plan-block').length;
  let yearlyIndex = document.querySelectorAll('#yearly-plans .plan-block').length;

  // ✅ Ajouter un plan
  document.querySelectorAll('.add-plan-button').forEach(button => {
    button.addEventListener('click', function () {
      const type = this.getAttribute('data-type'); // 'monthly' ou 'yearly'
      const templateId = type === 'monthly' ? 'vc-plan-template-monthly'
                      : type === 'yearly' ? 'vc-plan-template-yearly'
                      : 'vc-plan-template';
      const containerId = type === 'monthly' ? 'monthly-plans'
                      : type === 'yearly' ? 'yearly-plans'
                      : null;
...
if (type === 'monthly') {
  ...
  const toggleSections = () => {
    ...
    if (sub === 'no' && multi === 'no') {
      document.getElementById('pricing_simple_nosub')?.style?.setProperty('display', 'block');
    } else if (sub === 'no' && multi === 'yes') {
      document.getElementById('pricing_single_multi_nosub')?.style?.setProperty('display', 'block');
    } else if (sub === 'yes' && multi === 'no') {
      document.getElementById('pricing_simple_withsub')?.style?.setProperty('display', 'block');
    } else if (sub === 'yes' && multi === 'yes') {
      document.getElementById('pricing_multi_withsub')?.style?.setProperty('display', 'block');
    }
  };
...
document.addEventListener('DOMContentLoaded', function () {
  let generalIndex = document.querySelectorAll('#general-plans .plan-block').length;
